...
<script>
    ...
    document.addEventListener('livewire:init', function () {
        ...
        Livewire.on('service-created', () => {
            openToast('success', 'Service has created!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service-updated', () => {
            openToast('success', 'Service has updated!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service-update-error', () => {
            openToast('error', 'Service update error. Please try again!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service-deleted', () => {
            openToast('success', 'Service has deleted!')
        });
    });
    ...
    function view(id) {
        Livewire.navigate('/admin/services/'+id)
    }
