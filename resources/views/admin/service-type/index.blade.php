            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header pb-0 card-no-border">
                        <button class="btn btn-secondary mb-3" id="single-row-delete-btn" onclick="openModal()"><i class="fa fa-plus"></i> Add Service Type</button>
                    </div>
                    <div class="card-body">
...
<script>
    ...
    document.addEventListener('livewire:init', function () {
        ...
        Livewire.on('service_type-created', () => {
            openToast('success', 'Service type has created!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service_type-updated', () => {
            openToast('success', 'Service type has updated!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service_type-update-error', () => {
            openToast('error', 'Service type update error. Please try again!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service_type-deleted', () => {
            openToast('success', 'Service type has deleted!')
        });
    });
    ...
    function view(id) {
        Livewire.navigate('/admin/service_types/'+id)
    }
